...
By default, `cavecrew-reviewer` and `cavecrew-investigator` pin `model: haiku` in their frontmatter; `cavecrew-builder` has no `model:` line (uses the API session default). Set env vars in your shell before launching opencode to override per-agent:
...
Use the same model name strings you'd use in any opencode agent frontmatter (e.g. `haiku`, `sonnet`, `opus`).
