...
An opencode skill that compresses project memory files (`AGENTS.md`, todos,
preferences) into caveman format, reducing repeated input size.

opencode loads `AGENTS.md` on every session start, so large files add repeated
...
```
AGENTS.md          ← compressed (opencode reads smaller file each session)
...
`AGENTS.md` loads on every session start. A 1,000-token project memory file adds
...
- `caveman`: ask the model to answer in shorter prose
