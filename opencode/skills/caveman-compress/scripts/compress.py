...
def backup_dir_for(filepath: Path) -> Path:
    ...
    (opencode instructions/, rules/, etc.) stop re-ingesting the
...
def call_llm(prompt: str) -> str:
    ...
    Primary backend is the ``opencode run`` headless CLI, which authenticates
    through opencode's own auth store — no ``claude`` CLI and no
    ``ANTHROPIC_API_KEY`` required. When ``ANTHROPIC_API_KEY`` is set and the
    ``anthropic`` Python package is installed, the SDK is used directly as an
    optional fast path (no subprocess).

    ``--pure`` skips external plugins so the nested opencode process does not
    re-enter any installed plugin hooks. ``-m`` is passed through only when
    ``CAVEMAN_MODEL`` is set, using opencode's ``provider/model`` format.

    On Windows the subprocess decoding defaults to the system codepage
    ...
    if api_key:
        try:
            ...
            pass  # anthropic not installed, fall back to opencode CLI
    ...
    try:
        ...
        raise RuntimeError(
            "opencode CLI not found on PATH. Install opencode, or set "
            "ANTHROPIC_API_KEY (with the anthropic Python package) to use "
            "the SDK path."
        )
...
def compress_file(filepath: Path) -> bool:
    ...
    print("Compressing via opencode...")
    compressed_body = call_llm(build_compress_prompt(body))

    if compressed_body is None or not compressed_body.strip():
        print("❌ Compression aborted: LLM returned an empty response.")
    ...
    if compressed_body.strip() == body.strip():
        ...
        print("   Likely causes: LLM refused, returned the prompt verbatim, or the file is")
    ...
    for attempt in range(MAX_RETRIES):
        ...
        print("Fixing via opencode...")
        compressed = call_llm(
            build_fix_prompt(original_text, compressed, result.errors)
        )

        if compressed is None or not compressed.strip():
            print("❌ Fix attempt aborted: LLM returned an empty response.")
